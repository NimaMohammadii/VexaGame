...
async function ensureSoloTables(env:Env):Promise<void>{
  if(!soloSchemaReady){soloSchemaReady=(async()=>{
    await env.DB.prepare("CREATE TABLE IF NOT EXISTS mines_solo_rounds (round_id TEXT PRIMARY KEY,user_id TEXT NOT NULL,amount_nano INTEGER NOT NULL,mine_count INTEGER NOT NULL,bomb_cells_json TEXT NOT NULL,revealed_cells_json TEXT NOT NULL DEFAULT '[]',status TEXT NOT NULL,multiplier REAL NOT NULL DEFAULT 1,payout_nano INTEGER NOT NULL DEFAULT 0,settled INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)").run();
...
async function saveSoloRound(env:Env,round:SoloRound):Promise<void>{await ensureSoloTables(env);await env.DB.prepare(`INSERT INTO mines_solo_rounds (round_id,user_id,amount_nano,mine_count,bomb_cells_json,revealed_cells_json,status,multiplier,payout_nano,settled,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(round_id) DO UPDATE SET revealed_cells_json=excluded.revealed_cells_json,status=excluded.status,multiplier=excluded.multiplier,payout_nano=excluded.payout_nano,settled=excluded.settled,updated_at=CURRENT_TIMESTAMP`).bind(round.roundId,round.userId,round.amountNano,round.mines,JSON.stringify(round.bombs),JSON.stringify(round.revealed),round.status,round.multiplier,round.payoutNano,round.settled?1:0).run();}
