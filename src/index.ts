...
app.post('/app/api/pump/start', async (c) => {
  ...
  try {
    const body = await c.req.json().catch(() => ({})) as Record<string, unknown>;
    const auth = await authenticatedGameUser(c.env, body.initData, 'coinflip');
    userId = auth.userId;
    ...
    if (active) {
      ...
      return c.json({ ok: true, ...publicPumpRound(active), tonBalanceNano: controls.tonBalanceNano }, 200, { 'cache-control': 'no-store' });
    }
    roundId = `pump_${crypto.randomUUID().replace(/-/g, '').slice(0, 24)}`;
    const burstAt = secureRandomUnit() * 100 < auth.controls.winChancePercent ? 24 : 1.01;
    ...
    const inserted = await c.env.DB.prepare(`INSERT INTO pump_rounds
      ...
      VALUES (?,?,?, ?,1,0,'active',0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`).bind(roundId, userId, amountNano, burstAt).run();
    ...
    return c.json({ ok: true, ...publicPumpRound(row), tonBalanceNano: afterDebit.tonBalanceNano }, 200, { 'cache-control': 'no-store' });
  } catch (error) {
...
app.post('/app/api/pump/pump', async (c) => {
  try {
    ...
    const nextMultiplier = Math.round((Number(current.multiplier || 1) + 0.09 + Number(current.multiplier || 1) * 0.085 + nextPumps * 0.012) * 100) / 100;
