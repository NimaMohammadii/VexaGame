...
<section id="hilo" class="view cc-view" aria-label="Chicken Cross">
  ...
  (async function(){
    ...
    function createWorld(T,container){
      ...
      const asphalt=material(0x303236,.08,.77);texturedSurface(asphalt,'float aggregate=surfaceNoise(floor(surfaceUv*22.0));float fine=surfaceNoise(floor(surfaceUv*76.0));float wear=surfaceSoft(surfaceUv*1.35);diffuseColor.rgb*=.71+aggregate*.16+fine*.07+wear*.10;');
      const paving=material(0x69625e,0,.93);texturedSurface(paving,'vec2 tile=surfaceUv/vec2(1.55,1.1);tile.x+=mod(floor(tile.y),2.0)*.5;vec2 edge=abs(fract(tile)-.5);float mortar=1.0-smoothstep(.464,.495,max(edge.x,edge.y));float slab=surfaceNoise(floor(tile));float grain=surfaceNoise(floor(surfaceUv*24.0));diffuseColor.rgb*=(.76+slab*.20+grain*.09)*(1.0-mortar*.36);');
      const line=material(0xb8bcbc,0,.8),yellow=material(0xb9a675,0,.84),curbMat=material(0x96918a,0,.86),grass=material(0x253329,0,.98);
      ...
      const gutter=material(0x202226,.06,.46);for(const z of [9,-13.4])box(145,.012,.25,gutter,scene,0,-.047,z);
      ...
      for(const z of [9.3,-13.8]){box(145,.26,.34,curbMat,scene,0,.05,z);box(145,.12,3.4,paving,scene,0,.025,z+(z>0?1.85:-1.85));box(145,.04,5,grass,scene,0,-.02,z+(z>0?5.8:-5.8))}
      ...
      const cameraDistance=Math.hypot(28,13),roadWidth=20;
      ...
      camera.position.set(28,13,cameraZ);camera.lookAt(0,0,cameraZ);
