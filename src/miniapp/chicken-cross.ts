...
<section id="hilo" class="view cc-view" aria-label="Chicken Cross">
  ...
  (async function(){
    ...
    function createWorld(T,container,backdrop){
      ...
      const camera=new T.PerspectiveCamera(41,1,.2,170);const clock=new T.Clock();let running=false,frame=0,targetZ=9.8,shownZ=9.8,jumpStart=0,hitTime=0,hitCar=null,resolveCross=null,progressStep=0,blockedLanes=0;
      ...
      const reliefData=new Uint8Array(256*256*4);for(let i=0;i<reliefData.length;i+=4){const value=100+Math.floor(detailRandom()*95);reliefData[i]=reliefData[i+1]=reliefData[i+2]=value;reliefData[i+3]=255}
      const relief=new T.DataTexture(reliefData,256,256,T.RGBAFormat);relief.wrapS=relief.wrapT=T.RepeatWrapping;relief.magFilter=T.LinearFilter;relief.minFilter=T.LinearMipmapLinearFilter;relief.generateMipmaps=true;relief.needsUpdate=true;
      ...
      const bladeMaterial=new T.MeshStandardMaterial({color:0x667953,roughness:.96,side:T.DoubleSide,vertexColors:true});const lawn=new T.InstancedMesh(bladeGeometry,bladeMaterial,12000),detailTransform=new T.Object3D(),detailTint=new T.Color();lawn.receiveShadow=true;
      for(let i=0;i<12000;i++){const x=-45+detailRandom()*62,side=i%2?1:-1,z=side>0?12.95+detailRandom()*1.75:-17.5-detailRandom()*1.75;const patch=.55+.45*Math.sin(x*.73+z*2.1);detailTransform.position.set(x,.015,z);detailTransform.rotation.set(0,detailRandom()*Math.PI*2,0);detailTransform.scale.setScalar(.45+detailRandom()*.55+patch*.35);detailTransform.updateMatrix();lawn.setMatrixAt(i,detailTransform.matrix);detailTint.setRGB(.72+detailRandom()*.22,.76+detailRandom()*.20,.65+detailRandom()*.23);lawn.setColorAt(i,detailTint)}lawn.computeBoundingSphere();scene.add(lawn);
      ...
      mineral(bark,.04);const leafGeometry=new T.SphereGeometry(1,6,4),treeLeaves=new T.InstancedMesh(leafGeometry,leafMaterial,1600);treeLeaves.receiveShadow=true;treeLeaves.castShadow=true;let leafIndex=0;
      for(const [x,z,height] of [[-26,17.8,4.2],[-10,18.1,4.8]]){const trunk=mesh(new T.CylinderGeometry(.075,.19,height,12),bark,scene,x,height/2,z);trunk.castShadow=true;
        ...
        for(let leaf=0;leaf<800;leaf++){const angle=detailRandom()*Math.PI*2,vertical=detailRandom()*2-1,radius=Math.cbrt(detailRandom()),spread=Math.sqrt(1-vertical*vertical)*radius;detailTransform.position.set(x+Math.cos(angle)*spread*1.65,height+.15+vertical*radius*1.9,z+Math.sin(angle)*spread*1.4);detailTransform.rotation.set(detailRandom()*Math.PI,angle,detailRandom()*.9);detailTransform.scale.set(.12+detailRandom()*.09,.025,.055+detailRandom()*.04);detailTransform.updateMatrix();treeLeaves.setMatrixAt(leafIndex,detailTransform.matrix);detailTint.setRGB(.65+detailRandom()*.4,.75+detailRandom()*.3,.60+detailRandom()*.3);treeLeaves.setColorAt(leafIndex++,detailTint)}
      ...
      function car(color,direction){const group=new T.Group(),paint=material(color,.42,.29);scene.add(group);const shape=new T.Shape();shape.moveTo(-2.05,.38);shape.lineTo(-1.92,.80);shape.lineTo(-1.12,.91);shape.lineTo(-.65,1.48);shape.lineTo(.78,1.48);shape.lineTo(1.16,.91);shape.lineTo(1.95,.79);shape.lineTo(2.07,.42);shape.lineTo(2.07,.32);shape.lineTo(-2.05,.32);const shell=new T.ExtrudeGeometry(shape,{depth:1.54,bevelEnabled:true,bevelSegments:2,steps:1,bevelSize:.13,bevelThickness:.10,curveSegments:3});mesh(shell,paint,group,0,0,-.77).castShadow=true;
        ...
        const windshield=new T.Shape();windshield.moveTo(-.57,.02);windshield.lineTo(.57,.02);windshield.lineTo(.48,.39);windshield.lineTo(-.48,.39);windshield.closePath();const windGeom=new T.ShapeGeometry(windshield);windGeom.rotateY(Math.PI/2);mesh(windGeom,glass,group,1.175,1.03,0);
      ...
      for(let lane=0;lane<8;lane++){const z=9.2-(lane+.5)*2.84,obj=car(colors[lane],1),materials=[],clones=new Map();obj.name='traffic-car-'+(lane+1);obj.traverse(part=>{if(!part.isMesh)return;let cloned=clones.get(part.material);if(!cloned){cloned=part.material.clone();cloned.transparent=true;materials.push({mat:cloned,color:cloned.color.clone(),emissive:cloned.emissive&&cloned.emissive.clone()});clones.set(part.material,cloned)}part.material=cloned});obj.position.set(carStarts[lane],0,z);traffic.push({obj,z,speed:15,materials})}
      ...
      const barriers=[];for(let lane=0;lane<8;lane++){const group=new T.Group();group.name='lane-barrier-'+(lane+1);group.position.set(-4.4,-.052,9.2-(lane+.5)*2.84);group.visible=false;group.userData.amount=0;group.userData.target=0;scene.add(group);
        const plank=box(.28,.48,2.34,barrierRed,group,0,.72,0);plank.castShadow=true;
        for(let stripe=-2;stripe<=2;stripe++){const mark=box(.035,.31,.38,barrierWhite,group,.16,.72,stripe*.43);mark.rotation.x=-.55;mark.castShadow=true}
        for(const side of [-1,1]){const post=box(.18,.82,.16,barrierMetal,group,-.06,.43,side*.86);post.rotation.z=side*.10;post.castShadow=true;box(.72,.1,.18,barrierMetal,group,.08,.06,side*.88).castShadow=true;const lamp=mesh(new T.SphereGeometry(.10,12,8),barrierAmber,group,.02,1.02,side*.79);lamp.scale.set(1,.72,1);lamp.castShadow=true}
        barriers.push(group)}
      ...
      const featherGeometry=new T.SphereGeometry(1,32,24);
      ...
      function frameLoop(){if(!running)return;frame=requestAnimationFrame(frameLoop);const dt=Math.min(.05,clock.getDelta()),now=performance.now();for(let lane=0;lane<traffic.length;lane++){const car=traffic[lane];if(lane<blockedLanes){fadeCar(car,0);continue}if(hitTime&&car===hitCar)continue;car.obj.position.x+=car.speed*dt;if(car.obj.position.x>30)car.obj.position.x=-56;fadeCar(car,Math.min((car.obj.position.x+49)/13,(25-car.obj.position.x)/13))}
        for(const barrier of barriers){barrier.userData.amount+=(barrier.userData.target-barrier.userData.amount)*(1-Math.exp(-dt*11));if(Math.abs(barrier.userData.target-barrier.userData.amount)<.002)barrier.userData.amount=barrier.userData.target;barrier.visible=barrier.userData.amount>.01;barrier.scale.y=Math.max(.001,barrier.userData.amount)}
        ...
        bird.position.y=birdGround+hop;body.scale.y=1+Math.sin(now*.0022)*.008;head.rotation.y=Math.sin(now*.0008)*.08;head.rotation.x=Math.sin(now*.0016)*.027;
