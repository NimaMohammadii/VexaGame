...
<section id="hilo" class="view cc-view" aria-label="Chicken Cross">
  ...
  (async function(){
    ...
    function createWorld(T,container,backdrop){
      ...
      const barrierLampGeometry=new T.SphereGeometry(.10,10,7),barriers=[];for(let lane=0;lane<8;lane++){const group=new T.Group();group.name='lane-barrier-'+(lane+1);group.position.set(-4.4,-.052,9.2-(lane+.5)*2.84);group.visible=false;group.userData.amount=0;group.userData.target=0;scene.add(group);
      ...
      const birdGround=-.14;bird.position.set(0,birdGround,9.8);bird.rotation.y=0;const birdShadowMaterial=new T.MeshBasicMaterial({color:0x080608,transparent:true,opacity:.3,depthWrite:false}),birdShadow=mesh(new T.CircleGeometry(1,20),birdShadowMaterial,scene,0,-.045,9.8);birdShadow.rotation.x=-Math.PI/2;birdShadow.scale.set(.62,.4,1);birdShadow.receiveShadow=false;
