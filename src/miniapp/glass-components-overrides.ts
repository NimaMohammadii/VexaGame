...
button:not(.tab):not(.section-keyboard-dismiss):not(:where(.home-ticket-step,.home-ticket-button,.home-ticket-drawer-close,.home-bonus-close,.predict-zone-category-card,.predict-zone-choice,.predict-zone-bet-close,.predict-zone-bet-submit,.predict-zone-quick-bets>button,.shell-cup-button,.shell-play-button,.shell-preset,[data-shell-minus],[data-shell-plus])),.primary,.secondary,.ghost,.danger,.bot-row,.voice-btn,.voice-menu,.voice-menu button,.game-open,.plinko-drop,.plinko-quick button,.risk-segment,.risk-segment button,.rows-select,.bet-quick button,.autoplay-toggle,.crash-primary,.crash-secondary,.crash-quick button,.mine-tile,#minesStart,#minesCashout,[data-mines-bet],.top-balance-pill,.credit-pill,.pill{
  border:0!important;
  outline:0!important;
