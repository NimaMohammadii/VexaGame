...
(function(){
  ...
  ready(function(){
    ...
    function syncRound(my,id){
      ...
      }).catch(function(){if(my!==seq||id!==market||eventMode)return false;stopQuoteFeed();currentRound=null;roundLockDeadline=0;clearRoundMeta();syncBetAvailability();if(trendLabel)trendLabel.textContent='Price unavailable';return false})
    }
    ...
    function suspend(){if(runtimeSuspended)return;runtimeSuspended=true;stopClock();cancelDraw();stopQuoteFeed();stopFeed();setKeyboardOpen(false);closeBet(true)}
    function resume(){
      if(!isActive())return;
