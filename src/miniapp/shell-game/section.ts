...
<section id="shellgame" class="view shell-game-view" aria-label="Shell Game">
  <style>
    ...
    .shell-game-view{height:calc(100dvh - 52px - env(safe-area-inset-top));padding:0 10px max(22px,env(safe-area-inset-bottom));box-sizing:border-box;overflow:hidden;background-color:#030202;color:#fff}
  ...
  <script>(function(){
    ...
    minus.addEventListener('click',function(){if(state!=='ready')return;var index=0;for(var i=0;i<amounts.length;i++)if(amounts[i]<bet)index=i;setBet(amounts[index])});plus.addEventListener('click',function(){if(state!=='ready')return;var next=amounts[amounts.length-1];for(var i=0;i<amounts.length;i++)if(amounts[i]>bet){next=amounts[i];break}setBet(next)});presets.forEach(function(item){item.addEventListener('click',function(){if(state==='ready')setBet(Number(item.dataset.shellPreset))})});betButton.addEventListener('click',function(){if(state!=='ready')return;var rootTop=root.scrollTop,pageTop=window.pageYOffset||document.documentElement.scrollTop||0,value=window.prompt('Enter TON bet',money(bet));if(value!==null){var parsed=Number(String(value).replace(',','.'));if(Number.isFinite(parsed)&&parsed>0)setBet(parsed)}restorePromptViewport(rootTop,pageTop)});play.addEventListener('click',start);
