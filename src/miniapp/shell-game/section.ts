...
<section id="shellgame" class="view shell-game-view" aria-label="Shell Game">
  ...
  <div class="shell-game-wrap">
    ...
    <div class="shell-game-controls">
      ...
      <div class="shell-bet-row">
        ...
        <button class="shell-bet-value" type="button" data-shell-bet tabindex="-1"><img src="/app/api/uploaded-image/ton-icon.png" alt="GRAM"/><span>1</span><small>GRAM</small></button>
  ...
  <script>(function(){
    ...
    minus.addEventListener('click',function(){if(state!=='ready')return;var index=0;for(var i=0;i<amounts.length;i++)if(amounts[i]<bet)index=i;setBet(amounts[index])});plus.addEventListener('click',function(){if(state!=='ready')return;var next=amounts[amounts.length-1];for(var i=0;i<amounts.length;i++)if(amounts[i]>bet){next=amounts[i];break}setBet(next)});presets.forEach(function(item){item.addEventListener('click',function(){if(state==='ready')setBet(Number(item.dataset.shellPreset))})});betButton.addEventListener('click',function(event){if(state!=='ready')return;event.preventDefault();var current=money(bet);try{betButton.blur()}catch(e){}setTimeout(function(){if(state!=='ready'||!root.classList.contains('active'))return;var value=null;try{value=window.prompt('Enter GRAM bet',current)}catch(e){return}if(value!==null){var parsed=Number(String(value).replace(',','.'));if(Number.isFinite(parsed)&&parsed>0)setBet(parsed)}},180)});play.addEventListener('click',start);
