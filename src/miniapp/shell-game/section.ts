...
<section id="shellgame" class="view shell-game-view" aria-label="Shell Game">
  <style>
    ...
    .shell-bet-value{min-width:0;display:flex!important;align-items:center;justify-content:center;gap:8px;padding:0 10px;background:rgba(255,255,255,.035);color:#fff;overflow:hidden}.shell-bet-value img{width:29px;height:29px;object-fit:contain;flex:0 0 auto}.shell-bet-input{min-width:0;width:min(92px,28vw);padding:0;border:0;outline:0;background:transparent;color:#fff;text-align:center;font-family:var(--font-num);font-size:18px;font-weight:850;font-variant-numeric:tabular-nums lining-nums;font-feature-settings:"tnum" 1,"lnum" 1;caret-color:#fff}.shell-bet-input:disabled{opacity:.6}.shell-bet-value small{flex:0 0 auto;font-size:13px;color:rgba(255,255,255,.64)}
  ...
  <div class="shell-game-wrap">
    ...
    <div class="shell-game-controls">
      ...
      <div class="shell-bet-row">
        ...
        <label class="shell-bet-value"><img src="/app/api/uploaded-image/ton-icon.png" alt="TON"/><input class="shell-bet-input" type="text" inputmode="decimal" autocomplete="off" autocorrect="off" spellcheck="false" enterkeyhint="done" aria-label="Bet amount in TON" data-shell-bet-input value="1"/><small>TON</small></label>
  ...
  <script>(function(){
    ...
    function setControlsDisabled(disabled){minus.disabled=disabled;plus.disabled=disabled;betInput.disabled=disabled;presets.forEach(function(item){item.disabled=disabled})}
    ...
    minus.addEventListener('click',function(){if(state!=='ready')return;var index=0;for(var i=0;i<amounts.length;i++)if(amounts[i]<bet)index=i;setBet(amounts[index])});plus.addEventListener('click',function(){if(state!=='ready')return;var next=amounts[amounts.length-1];for(var i=0;i<amounts.length;i++)if(amounts[i]>bet){next=amounts[i];break}setBet(next)});presets.forEach(function(item){item.addEventListener('click',function(){if(state==='ready')setBet(Number(item.dataset.shellPreset))})});betInput.addEventListener('focus',function(){betFocusScrollTop=root.scrollTop});betInput.addEventListener('input',function(){var clean=cleanBetDraft(betInput.value);if(clean!==betInput.value)betInput.value=clean});betInput.addEventListener('blur',function(){applyBetInput();restoreBetScroll()});betInput.addEventListener('keydown',function(event){if(event.key==='Enter'){event.preventDefault();applyBetInput();betInput.blur()}});play.addEventListener('click',start);
    window.addEventListener('vexa:view-changed',function(event){if(!event.detail||event.detail.id==='shellgame')return;timers.forEach(clearTimeout);timers=[];state='ready';resetCups();play.disabled=false;setControlsDisabled(false);playLabel.textContent='Play Round';setStatus('Press Play Round to begin')});
    setBet(1);resetCups();
  })();</script>
</section>`;
