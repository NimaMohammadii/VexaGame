...
(function(){
  ...
  function telegramInitData(){return String(tg&&tg.initData||'').trim()}
  async function api(path,payload){var r=await fetch(path,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload||{})});var j=await r.json().catch(function(){return{error:'Invalid response'}});if(!r.ok)throw new Error(j.error||'Request failed');return j}
  async function getJson(path){var initData=telegramInitData();if(!initData)throw new Error('Open the Mini App inside Telegram');var r=await fetch(path,{headers:{accept:'application/json','x-telegram-init-data':initData},cache:'no-store'});var j=await r.json().catch(function(){return{error:'Invalid response'}});if(!r.ok)throw new Error(j.error||'Request failed');return j}
  ...
  function savePending(deposit){try{localStorage.setItem(STORAGE_KEY,JSON.stringify({id:String(deposit.id||''),userId:ownerId(),network:String(deposit.network||''),createdAt:Date.now()}))}catch(e){}}
