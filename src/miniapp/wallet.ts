export const WALLET_SECTION = `<div id="wallet" class="wallet-sheet" aria-hidden="true">
  <style>
    #wallet.wallet-sheet{position:fixed!important;inset:0!important;bottom:auto!important;height:var(--vexa-wallet-sheet-height,100%)!important;z-index:10070!important;display:block!important;pointer-events:none!important;visibility:hidden!important}
    ...
    body.wallet-open.deposit-open #wallet .wallet-sheet-panel{height:min(420px,calc(var(--vexa-wallet-sheet-height,100vh) - 16px))!important}
    body.wallet-open.withdraw-open #wallet .wallet-sheet-panel{height:min(452px,calc(var(--vexa-wallet-sheet-height,100vh) - 16px))!important}

    body.wallet-open.deposit-open #depositSheet.deposit-sheet.open,body.wallet-open.withdraw-open #withdrawSheet.deposit-sheet.open{position:fixed!important;inset:0!important;bottom:auto!important;z-index:10072!important;width:100%!important;height:var(--vexa-wallet-sheet-height,100%)!important;min-height:0!important;padding:0!important;margin:0!important;display:block!important;background:transparent!important;opacity:1!important;visibility:visible!important;pointer-events:auto!important;will-change:transform!important;transform:translateY(calc(0px - var(--vexa-wallet-keyboard-offset,0px)))!important;transition:transform .48s cubic-bezier(.22,1,.36,1)!important}
    ...
    body.wallet-open.deposit-open #depositSheet.deposit-sheet .deposit-close,body.wallet-open.withdraw-open #withdrawSheet.deposit-sheet .deposit-close{width:44px!important;height:44px!important;min-width:44px!important;padding:0!important;border:1px solid rgba(255,255,255,.10)!important;border-radius:14px!important;background:linear-gradient(145deg,#202027,#15151a)!important;color:rgba(255,255,255,.92)!important;display:grid!important;place-items:center!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.045)!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;transition:transform .20s ease,background .20s ease,border-color .20s ease!important}
    body.wallet-open.deposit-open #depositSheet .deposit-close svg,body.wallet-open.withdraw-open #withdrawSheet.deposit-sheet .deposit-close svg{width:18px!important;height:18px!important;stroke-width:1.8!important}
    ...
    body.wallet-open.deposit-open #depositSheet .deposit-method-option{min-height:68px!important;border:1px solid rgba(255,255,255,.10)!important;border-radius:18px!important;padding:10px 12px!important;background:linear-gradient(135deg,rgba(255,255,255,.065),rgba(255,255,255,.025))!important;background-image:linear-gradient(135deg,rgba(255,255,255,.065),rgba(255,255,255,.025))!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.04)!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;transition:transform .20s ease,border-color .20s ease,background .20s ease!important}
    body.wallet-open.deposit-open #depositSheet .deposit-method-option:active{border-color:rgba(174,132,149,.58)!important;background:linear-gradient(135deg,rgba(103,48,67,.34),rgba(255,255,255,.035))!important;transform:scale(.992)!important}
    ...
    @media(max-width:380px){body.wallet-open.deposit-open #wallet .wallet-sheet-panel{height:min(400px,calc(var(--vexa-wallet-sheet-height,100vh) - 14px))!important}body.wallet-open.withdraw-open #wallet .wallet-sheet-panel{height:min(430px,calc(var(--vexa-wallet-sheet-height,100vh) - 14px))!important}body.wallet-open.deposit-open #depositSheet.deposit-sheet.open .deposit-panel{height:min(400px,calc(var(--vexa-wallet-sheet-height,100vh) - 14px))!important;max-height:min(400px,calc(var(--vexa-wallet-sheet-height,100vh) - 14px))!important}body.wallet-open.withdraw-open #withdrawSheet.deposit-sheet.open .deposit-panel{height:min(430px,calc(var(--vexa-wallet-sheet-height,100vh) - 14px))!important;max-height:min(430px,calc(var(--vexa-wallet-sheet-height,100vh) - 14px))!important}body.wallet-open.deposit-open #depositSheet .deposit-method-option{min-height:64px!important;padding:9px 11px!important}}
...
(function(){
  ...
  function closeWallet(){setDepositKeyboard(false);setSheet('withdrawSheet',false);setSheet('depositSheet',false);if(window.VexaTonUsdPrice&&typeof window.VexaTonUsdPrice.stop==='function')window.VexaTonUsdPrice.stop();var sheet=q('wallet');document.body.classList.remove('wallet-open');if(sheet){sheet.classList.remove('open');sheet.setAttribute('aria-hidden','true')}document.documentElement.style.removeProperty('--vexa-wallet-sheet-height');document.documentElement.style.removeProperty('--vexa-wallet-keyboard-offset');var back=tg&&tg.BackButton;try{back&&back.offClick&&back.offClick(closeWallet)}catch(e){}try{back&&back.hide&&back.hide()}catch(e){}}
  function openWallet(){var sheet=ensureWallet();if(!sheet)return;if(sheet.parentElement!==document.body)document.body.appendChild(sheet);document.documentElement.style.setProperty('--vexa-wallet-sheet-height',Math.max(window.innerHeight||0,document.documentElement.clientHeight||0)+'px');document.documentElement.style.setProperty('--vexa-wallet-keyboard-offset','0px');document.body.classList.add('wallet-open');sheet.classList.add('open');sheet.setAttribute('aria-hidden','false');setSheet('depositSheet',true);clearDepositStatus();updateEquivalent();if(window.VexaStarsGramRate&&typeof window.VexaStarsGramRate.load==='function')window.VexaStarsGramRate.load();if(window.VexaShowDepositPicker)window.VexaShowDepositPicker();var back=tg&&tg.BackButton;try{back&&back.offClick&&back.offClick(closeWallet)}catch(e){}try{back&&back.onClick&&back.onClick(closeWallet);back&&back.show&&back.show()}catch(e){}}
  ...
  function syncDepositKeyboardPosition(){var root=document.documentElement;var viewport=window.visualViewport;var open=document.body.classList.contains('deposit-keyboard-open');var base=Number.parseFloat(root.style.getPropertyValue('--vexa-wallet-sheet-height'))||Math.max(window.innerHeight||0,document.documentElement.clientHeight||0);var offset=0;if(open){offset=viewport?Math.max(0,Math.round(base-viewport.height-viewport.offsetTop)):Math.max(0,Math.round(base-(window.innerHeight||base)))}root.style.setProperty('--vexa-wallet-keyboard-offset',offset+'px')}
