...
(function(){
  ...
  function ensureWithdrawMethods(){var sheet=q('withdrawSheet');if(!sheet)return;var style=q('vexa-withdraw-method-style');if(!style){style=document.createElement('style');style.id='vexa-withdraw-method-style';style.textContent='#withdrawSheet .withdraw-method-grid{width:min(100%,340px)!important;margin:0 auto 12px!important;display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important}.withdraw-method-button{appearance:none!important;-webkit-appearance:none!important;min-height:44px!important;border:0!important;border-radius:17px!important;padding:7px 10px!important;background:rgba(0,0,0,.20)!important;color:#fff!important;display:grid!important;place-items:center!important;gap:2px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.06)!important;transition:transform .2s ease,background .2s ease,box-shadow .2s ease!important}.withdraw-method-button:active{transform:scale(.985)!important}.withdraw-method-button strong{font-size:13px!important;font-weight:950!important;line-height:1!important;letter-spacing:-.02em!important}.withdraw-method-button span{font-size:9.5px!important;font-weight:760!important;color:rgba(255,255,255,.50)!important}.withdraw-method-button.selected{background:linear-gradient(135deg,rgba(92,14,36,.84),rgba(45,6,19,.88))!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.10),0 10px 22px rgba(0,0,0,.16)!important}.withdraw-method-button.selected span{color:rgba(255,255,255,.72)!important}#withdrawSheet .deposit-ton-equivalent{white-space:nowrap!important;color:rgba(255,255,255,.72)!important;font-size:12px!important;font-weight:800!important}';document.head.appendChild(style)}var copy=sheet.querySelector('.deposit-copy');if(!q('withdrawMethodGrid')&&copy){var grid=document.createElement('div');grid.id='withdrawMethodGrid';grid.className='withdraw-method-grid';grid.innerHTML='<button class="withdraw-method-button selected" type="button" data-action="select-withdraw-method" data-withdraw-method="gram"><strong>GRAM</strong><span>Native wallet</span></button><button class="withdraw-method-button" type="button" data-action="select-withdraw-method" data-withdraw-method="usdt-bep20"><strong>USDT</strong><span>BEP20</span></button>';copy.insertAdjacentElement('afterend',grid)}}
