export const WALLET_SECTION = `<div id="wallet" class="wallet-sheet" aria-hidden="true">
  <style>
    ...
    body.wallet-open.deposit-open #wallet .wallet-sheet-panel:before{height:min(480px,calc(100vh - 16px))!important;background:rgba(18,18,19,.66)!important}
    ...
    body.wallet-open.deposit-open #depositSheet .deposit-close svg,body.wallet-open.withdraw-open #withdrawSheet.deposit-sheet .deposit-close svg{width:20px!important;height:20px!important}
    ...
    #wallet .wallet-sheet-panel,#depositSheet :is(h1,h2,h3,h4,strong,b,button,input),#withdrawSheet :is(h1,h2,h3,h4,strong,b,button,input),#transactionsSheet :is(h1,h2,h3,h4,strong,b,button,input,em){font-family:ui-rounded,"SF Pro Rounded","SF Pro Display",-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;font-weight:700!important;letter-spacing:-.02em!important;font-variant-numeric:tabular-nums!important}
  ...
  <script>
    (function(){
      ...
      function pickMode(method,btn){
        if(method==='nft'){
          btn.animate([{transform:'scale(1)'},{transform:'scale(.97)'},{transform:'scale(1)'}],{duration:300,easing:'cubic-bezier(.18,.9,.22,1.25)'});
          return;
        }
...
(function(){
  ...
  function ensureWithdrawMethods(){var sheet=q('withdrawSheet');if(!sheet)return;var style=q('vexa-withdraw-method-style');if(!style){style=document.createElement('style');style.id='vexa-withdraw-method-style';style.textContent='#withdrawSheet .withdraw-method-grid{width:min(100%,340px)!important;margin:0 auto 12px!important;display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important}.withdraw-method-button{appearance:none!important;-webkit-appearance:none!important;min-height:44px!important;border:0!important;border-radius:16px!important;padding:7px 10px!important;background:rgba(255,255,255,.035)!important;color:rgba(255,255,255,.48)!important;opacity:.62!important;display:grid!important;place-items:center!important;gap:2px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.045)!important;transform:scale(.985)!important;will-change:transform,opacity,background!important;transition:transform .34s cubic-bezier(.16,1,.3,1),opacity .28s ease,background .28s ease,color .28s ease,box-shadow .28s ease!important}.withdraw-method-button strong{font-size:13px!important;font-weight:900!important;line-height:1!important;letter-spacing:-.02em!important;color:inherit!important}.withdraw-method-button span{font-size:9.5px!important;font-weight:720!important;color:rgba(255,255,255,.34)!important;transition:color .28s ease!important}.withdraw-method-button.selected{background:rgba(255,255,255,.09)!important;color:#fff!important;opacity:1!important;transform:translateY(-1px) scale(1.015)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.10),0 8px 20px rgba(0,0,0,.16)!important}.withdraw-method-button.selected span{color:rgba(255,255,255,.66)!important}.withdraw-method-button:active{transform:scale(.965)!important;transition-duration:.12s!important}#withdrawSheet .deposit-ton-equivalent{white-space:nowrap!important;color:rgba(255,255,255,.72)!important;font-size:12px!important;font-weight:800!important}';document.head.appendChild(style)}var copy=sheet.querySelector('.deposit-copy');if(!q('withdrawMethodGrid')&&copy){var grid=document.createElement('div');grid.id='withdrawMethodGrid';grid.className='withdraw-method-grid';grid.innerHTML='<button class="withdraw-method-button selected" type="button" data-action="select-withdraw-method" data-withdraw-method="gram"><strong>GRAM</strong><span>Native wallet</span></button><button class="withdraw-method-button" type="button" data-action="select-withdraw-method" data-withdraw-method="usdt-bep20"><strong>USDT</strong><span>BEP20</span></button>';copy.insertAdjacentElement('afterend',grid)}}
  function setWithdrawMethod(method){withdrawMethod=method==='usdt-bep20'?'usdt-bep20':'gram';ensureWithdrawMethods();var sheet=q('withdrawSheet');if(!sheet)return;Array.prototype.forEach.call(sheet.querySelectorAll('.withdraw-method-button'),function(button){button.classList.toggle('selected',button.getAttribute('data-withdraw-method')===withdrawMethod)});var title=sheet.querySelector('.deposit-title-main h3,.deposit-title h3,h3');if(title)title.textContent='Withdraw';var fields=sheet.querySelectorAll('.deposit-custom-field');var amountLabel=fields[0]&&fields[0].querySelector('label');var walletLabel=fields[1]&&fields[1].querySelector('label');if(amountLabel)amountLabel.textContent='GRAM Amount';if(walletLabel)walletLabel.textContent=withdrawMethod==='usdt-bep20'?'USDT Wallet Address':'GRAM Wallet Address';var wallet=q('withdrawWalletAddress');if(wallet){wallet.placeholder=withdrawMethod==='usdt-bep20'?'0x... BEP20 address':'UQ... wallet address';wallet.setAttribute('autocomplete','off');wallet.setAttribute('autocorrect','off')}var copy=sheet.querySelector('.deposit-copy');if(copy)copy.textContent=withdrawMethod==='usdt-bep20'?'Enter your BEP20 wallet and the GRAM amount to withdraw as USDT':'Enter your GRAM wallet and withdrawal amount';updateWithdrawEquivalent()}
  ...
  function updateWithdrawEquivalent(){var out=q('withdrawUsdEquivalent');var amount=tonAmount(q('withdrawAmountTon')&&q('withdrawAmountTon').value);if(withdrawMethod==='usdt-bep20'){if(!out)return;if(!tonUsd&&amount)loadTonUsd();out.textContent=(tonUsd&&amount)?'≈ '+(amount*tonUsd).toFixed(2)+' USDT':(amount?'≈ … USDT':'≈ 0.00 USDT');return}setUsdEquivalent(out,amount)}
...
(function(){
  ...
  function installStyles(){if(q('vexa-usdt-deposit-style'))return;var style=document.createElement('style');style.id='vexa-usdt-deposit-style';style.textContent='#depositSheet.deposit-usdt-enabled.deposit-choosing .deposit-method-option{min-height:60px!important;padding:7px 10px!important;grid-template-columns:46px minmax(0,1fr) 20px!important;gap:10px!important}#depositSheet.deposit-usdt-enabled.deposit-choosing .deposit-method-image{width:46px!important;height:46px!important}#depositSheet.deposit-usdt-enabled.deposit-choosing .deposit-method-image img{width:44px!important;height:44px!important}#depositSheet.deposit-usdt-enabled.deposit-choosing .deposit-method-option[data-method="ton"] .deposit-method-image img{width:52px!important;height:52px!important}#depositSheet.deposit-usdt-enabled.deposit-choosing .deposit-method-copybox strong{font-size:15.5px!important}#depositSheet.deposit-usdt-enabled.deposit-choosing .deposit-method-copybox span{font-size:10.5px!important}#depositSheet.deposit-usdt-enabled.deposit-choosing .deposit-method-copy{margin-bottom:8px!important}.deposit-method-option[data-action="choose-usdt-deposit"]{background:linear-gradient(145deg,rgba(126,28,58,.88),rgba(57,8,25,.92) 52%,rgba(24,3,11,.96))!important;box-shadow:0 14px 30px rgba(50,3,19,.44),inset 1px 1px 0 rgba(255,255,255,.16),inset -2px -2px 4px rgba(19,0,7,.74),inset 0 0 10px rgba(169,39,79,.22)!important}.usdt-method-icon{width:44px!important;height:44px!important;display:block!important;color:#fff!important}.usdt-deposit-screen{display:none!important;width:min(100%,360px)!important;margin:0 auto!important}.deposit-usdt-mode .usdt-deposit-screen{display:block!important}#depositSheet.deposit-usdt-mode .deposit-method-screen,#depositSheet.deposit-usdt-mode .deposit-copy:not(.deposit-method-copy),#depositSheet.deposit-usdt-mode .deposit-custom-field,#depositSheet.deposit-usdt-mode .deposit-action-row,#depositSheet.deposit-usdt-mode #depositMainPayButton,#depositSheet.deposit-usdt-mode .ton-wallet-status,#depositSheet.deposit-usdt-mode .deposit-status-inline,#depositSheet.deposit-usdt-mode .deposit-stars-logo,#depositSheet.deposit-usdt-mode #tonWalletConnectGate{display:none!important}.usdt-deposit-head{text-align:center!important;margin:0 0 14px!important}.usdt-deposit-head strong{display:block!important;font-size:18px!important;font-weight:950!important;letter-spacing:-.04em!important;color:#fff!important}.usdt-deposit-head span{display:block!important;margin-top:5px!important;font-size:11px!important;font-weight:720!important;line-height:1.35!important;color:rgba(255,255,255,.55)!important}.usdt-network-grid{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important;margin:0 0 10px!important}.usdt-network-button{height:42px!important;border:0!important;border-radius:16px!important;background:rgba(255,255,255,.035)!important;color:rgba(255,255,255,.46)!important;font-size:12px!important;font-weight:850!important;opacity:.62!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.045)!important;transform:scale(.985)!important;will-change:transform,opacity,background!important;transition:transform .34s cubic-bezier(.16,1,.3,1),opacity .28s ease,background .28s ease,color .28s ease,box-shadow .28s ease!important}.usdt-network-button.selected{background:rgba(255,255,255,.09)!important;color:#fff!important;opacity:1!important;transform:translateY(-1px) scale(1.015)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.10),0 8px 20px rgba(0,0,0,.16)!important}.usdt-network-button:active{transform:scale(.965)!important;transition-duration:.12s!important}.usdt-amount-field{height:46px!important;display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;align-items:center!important;gap:10px!important;padding:0 15px!important;border-radius:17px!important;background:rgba(0,0,0,.20)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.06)!important}.usdt-amount-field input{width:100%!important;height:100%!important;border:0!important;outline:0!important;background:transparent!important;color:#fff!important;font-size:17px!important;font-weight:800!important}.usdt-amount-field span{font-size:12px!important;font-weight:900!important;color:rgba(255,255,255,.64)!important}.usdt-primary{width:100%!important;height:44px!important;margin:10px 0 0!important;border:0!important;border-radius:18px!important;background:linear-gradient(145deg,#8d2445 0%,#500d26 48%,#22040f 100%)!important;color:#fff!important;font-size:13px!important;font-weight:950!important;box-shadow:0 12px 28px rgba(55,3,21,.48),inset 1px 1px 0 rgba(255,255,255,.18),inset -2px -2px 3px rgba(25,0,9,.76),inset 0 0 9px rgba(177,43,83,.26)!important}.usdt-primary:disabled,.usdt-copy-button:disabled{opacity:.55!important}.usdt-deposit-status{min-height:17px!important;margin:8px 0 0!important;text-align:center!important;font-size:11px!important;font-weight:760!important;color:rgba(255,255,255,.58)!important}.usdt-deposit-status.pending{color:#ffcf6b!important}.usdt-deposit-status.success{color:#42f594!important}.usdt-deposit-status.error{color:#ff7b9a!important}.usdt-quote{display:none!important}.usdt-deposit-screen.has-quote .usdt-create{display:none!important}.usdt-deposit-screen.has-quote .usdt-quote{display:block!important}.usdt-quote-amount{text-align:center!important;margin:0 0 10px!important}.usdt-quote-amount small{display:block!important;color:rgba(255,255,255,.48)!important;font-size:10px!important;font-weight:800!important;text-transform:uppercase!important;letter-spacing:.08em!important}.usdt-quote-amount strong{display:block!important;margin-top:4px!important;color:#fff!important;font-size:25px!important;font-weight:950!important;letter-spacing:-.045em!important;font-variant-numeric:tabular-nums!important}.usdt-quote-amount span{display:block!important;margin-top:3px!important;color:rgba(255,255,255,.55)!important;font-size:11px!important;font-weight:760!important}.usdt-address-box{display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;gap:8px!important;align-items:center!important;padding:10px 10px 10px 12px!important;border-radius:17px!important;background:rgba(0,0,0,.20)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.055)!important}.usdt-address-text{min-width:0!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;color:rgba(255,255,255,.84)!important;font-size:11px!important;font-weight:760!important;font-family:ui-monospace,SFMono-Regular,Menlo,monospace!important}.usdt-copy-button{height:34px!important;padding:0 12px!important;border:0!important;border-radius:12px!important;background:linear-gradient(145deg,#761b38 0%,#3a0818 52%,#18030b 100%)!important;color:#fff!important;font-size:10px!important;font-weight:900!important;box-shadow:0 8px 18px rgba(45,3,17,.38),inset 1px 1px 0 rgba(255,255,255,.15),inset -2px -2px 3px rgba(20,0,7,.7)!important}.usdt-quote-actions{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important;margin-top:8px!important}.usdt-note{margin:9px auto 0!important;max-width:330px!important;text-align:center!important;color:rgba(255,255,255,.48)!important;font-size:10px!important;font-weight:700!important;line-height:1.35!important}';document.head.appendChild(style)}
  function usdtIcon(){return '<svg class="usdt-method-icon" viewBox="0 0 48 48" fill="none" aria-hidden="true"><circle cx="24" cy="24" r="20" fill="rgba(255,255,255,.08)"/><path d="M15 15h18M24 15v19M18 21.5c3.7 2 8.3 2 12 0" stroke="currentColor" stroke-width="2.6" stroke-linecap="round"/><path d="M16.5 27c4.7 2.35 10.3 2.35 15 0" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" opacity=".7"/></svg>'}
