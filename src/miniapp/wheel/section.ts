...
<section id="wheel" class="view wheel-view">
  <style>
    ...
    /* Wheel mode switch */
    .wheel-mode-switch{position:absolute;z-index:4;top:10px;right:8px;width:48px;height:26px;margin:0;padding:3px;display:grid;grid-template-columns:1fr 1fr;border:1px solid rgba(255,255,255,.16);border-radius:999px;background:rgba(9,6,9,.72);box-shadow:0 8px 20px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.08);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}
    .wheel-mode-switch:before{content:"";position:absolute;left:3px;top:3px;width:20px;height:20px;border-radius:50%;background:linear-gradient(145deg,#8a2c54,#400d26);box-shadow:0 4px 10px rgba(31,0,10,.45),inset 0 1px 0 rgba(255,255,255,.18);transition:transform .42s cubic-bezier(.2,.8,.2,1),background .42s ease}
    .wheel-view.daily-mode .wheel-mode-switch:before{transform:translateX(22px);background:linear-gradient(145deg,#9a4066,#4a173a)}
    .wheel-mode-button{position:relative;z-index:1;border:0;background:transparent;color:transparent;font-size:0;transition:transform .3s ease}
    .wheel-mode-button:active{transform:scale(.9)}
    ...
    @media(prefers-reduced-motion:reduce){.wheel-mode-panel,.wheel-mode-switch:before{transition-duration:.01ms!important}}
  ...
  <div class="wheel-mode-switch" role="tablist" aria-label="Wheel mode">
    <button class="wheel-mode-button active" type="button" role="tab" aria-label="Classic wheel mode" aria-selected="true" data-wheel-mode="classic"></button>
    <button class="wheel-mode-button" type="button" role="tab" aria-label="Daily wheel mode" aria-selected="false" data-wheel-mode="daily"></button>
  </div>
  ...
  <script>
    (function(){
      function initWheelGame(){
        ...
        var dailyPrizes=['4 GRAM','2x WIN · 24H','30% Cashback','60% Deposit Bonus','5 Lottery Tickets','Empty','0.5 GRAM','Free Predict · 1 Play','Lottery Ticket','Empty','0.1 GRAM','10% Cashback'];
        ...
        function spinDailyWheel(){
          ...
          dailyRotor.style.transition='transform 7.2s cubic-bezier(.11,.72,.12,1)';
          requestAnimationFrame(function(){dailyRotor.style.transform='rotate('+dailyRotation+'deg)'});
          setTimeout(finishDailySpin,7700)
        }
